...
contract SnapshotUserRegistry is Ownable, IUserRegistry {
    ...
    function setStorageRoot(
        ...
        StateProofVerifier.Account memory account =
            StateProofVerifier.extractAccountFromProof(addressHash, _stateRoot, proof);
     ...
     */
    function addUser(address _user, bytes memory storageProofRlpBytes) external {
        ...
        StateProofVerifier.SlotValue memory slotValue =
            StateProofVerifier.extractSlotValueFromProof(proofPath, storageRoot, proof);
        require(slotValue.exists, "SnapshotUserRegistry: User is not qualified");
        require(
            slotValue.value >= minBalance, "SnapshotUserRegistry: User did not meet the minimum balance requirement"
        );
