...
contract PermissionedRecipientRegistry is Ownable, BaseRecipientRegistry {
    ...
    event RequestResolved( // Only for accepted registration requests
        bytes32 indexed _recipientId,
        RequestType indexed _type,
     ...
     */
    constructor(uint256 _baseDeposit, uint256 _challengePeriodDuration, address _controller) public {
     ...
     */
    function setChallengePeriodDuration(uint256 _challengePeriodDuration) external onlyOwner {
        challengePeriodDuration = _challengePeriodDuration;
    }
     ...
     */
    function addRecipient(address _recipient, string calldata _metadata) external payable {
        ...
        requests[recipientId] =
            Request(RequestType.Registration, payable(msg.sender), block.timestamp, msg.value, _recipient, _metadata);
        emit RequestSubmitted(recipientId, RequestType.Registration, _recipient, _metadata, block.timestamp);
    }
    ...
    function removeRecipient(bytes32 _recipientId) external payable {
        ...
        require(msg.value == baseDeposit, "RecipientRegistry: Incorrect deposit amount");
        requests[_recipientId] =
            Request(RequestType.Removal, payable(msg.sender), block.timestamp, msg.value, address(0), "");
        emit RequestSubmitted(_recipientId, RequestType.Removal, address(0), "", block.timestamp);
    }
     ...
     */
    function challengeRequest(bytes32 _recipientId, address payable _beneficiary) external onlyOwner returns (bool) {
        ...
        emit RequestResolved(_recipientId, request.requestType, true, 0, block.timestamp);
        return isSent;
    }
     ...
     * @param _recipientId The ID of recipient.
     */
    function executeRequest(bytes32 _recipientId) external onlyOwner returns (bool) {
        Request memory request = requests[_recipientId];
        require(request.submissionTime != 0, "RecipientRegistry: Request does not exist");
        ...
        emit RequestResolved(_recipientId, request.requestType, false, recipientIndex, block.timestamp);
        return isSent;
    }
}
