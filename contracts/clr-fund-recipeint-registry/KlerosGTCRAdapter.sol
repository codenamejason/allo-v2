...
contract KlerosGTCRAdapter is BaseRecipientRegistry {
    ...
    // Events
    event RecipientAdded(bytes32 indexed _tcrItemId, bytes _metadata, uint256 _index, uint256 _timestamp);
    event RecipientRemoved(bytes32 indexed _tcrItemId, uint256 _timestamp);
     ...
     */
    constructor(IKlerosGTCR _tcr, address _controller) public {
        tcr = _tcr;
     ...
     */
    function addRecipient(bytes32 _tcrItemId) external {
     ...
     * @param _tcrItemId The ID of the TCR item.
     */
    function removeRecipient(bytes32 _tcrItemId) external {
